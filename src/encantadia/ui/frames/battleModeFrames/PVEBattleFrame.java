...
public class PVEBattleFrame extends JFrame {
        ...
        for (Object[] b : bindings) {
            ...
            am.put(id, new AbstractAction() {
                @Override public void actionPerformed(java.awt.event.ActionEvent e) {
                    if (si < skillBtns.length && skillBtns[si] != null && skillBtns[si].isEnabled())
                        onPlayerSkill(si);
                }
            });
        }
    ...
    private void endRound(boolean playerWon) {
        ...
        if (playerWins >= ROUNDS_TO_WIN || enemyWins >= ROUNDS_TO_WIN) {
            new Timer(1600, e -> { ((Timer)e.getSource()).stop(); showMatchResult(playerWins >= ROUNDS_TO_WIN); })
    ...
    private void showMatchResult(boolean playerWon) {
        ...
        log("★  MATCH OVER — " + winner + " wins " + Math.max(playerWins, enemyWins)
                + "–" + Math.min(playerWins, enemyWins) + "!");
    ...
    private void onMatchTimerExpired() {
        ...
        if (playerCharacter.getCurrentHP() > enemyCharacter.getCurrentHP()) {
            log("🏆  " + playerCharacter.getName() + " wins by HP advantage!");
            endRound(true);
        } else if (enemyCharacter.getCurrentHP() > playerCharacter.getCurrentHP()) {
            log("💀  " + enemyCharacter.getName() + " wins by HP advantage!");
            endRound(false);
        } else {
            log("⚖  Draw — " + enemyCharacter.getName() + " claims the round.");
            endRound(false);
        }
    }
    ...
    private class BattleCanvas extends JPanel {
        ...
        @Override protected void paintComponent(Graphics g) {
            ...
            drawShadow(g2, "HP: " + playerCharacter.getCurrentHP() + " / " + playerCharacter.getMaxHP(),
                    phx, phy + hpH + (int)(10*sc), new Color(0xFF,0xF5,0xDC,190));
            drawNamePill(g2, playerCharacter.getName(), phx, phy + hpH + (int)(14*sc), pillW, pillH,
            ...
            g2.setFont(new Font("SansSerif", Font.PLAIN, Math.max(8, (int)(10*sc))));
            drawShadow(g2, "HP: " + enemyCharacter.getCurrentHP() + " / " + enemyCharacter.getMaxHP(),
                    ehx, ehy + hpH + (int)(10*sc), new Color(0xFF,0xF5,0xDC,190));
            drawNamePill(g2, enemyCharacter.getName(), ehx, ehy + hpH + (int)(14*sc), pillW, pillH,
                    new Color(0x50,0x10,0x10,215), new Color(0xFF,0x77,0x77));
            g2.setFont(new Font("SansSerif", Font.BOLD, Math.max(9, (int)(11*sc))));
        ...
        // ── NEW flanking timer badges ─────────────────────────
        /**
         * Draws two badges that sit flush within the top-bar area:
         *  LEFT  of the round tablet → Match timer (MM:SS, gold)
         *  RIGHT of the round tablet → Turn countdown (pulsing)
         * Both are vertically centred to the tablet height.
         */
        private void drawFlankTimers(Graphics2D g2, int W,
                                     int tabX, int tabY, int tabW, int tabH, double sc) {
            int padH = (int)(12 * sc), padV = (int)(5 * sc);

            // ── Match timer (LEFT flank) ──────────────────────
            ...
            int mW = mfm.stringWidth(mTxt) + padH * 2;
            int mH = mfm.getHeight() + padV * 2;
            // Clamp so it never overlaps the tablet
            int mX = Math.max((int)(6*sc), tabX - mW - (int)(8*sc));
            int mY = tabY + (tabH - mH) / 2;
            if (mX < (int)(4*sc)) mX = (int)(4*sc); // safe left edge

            g2.setColor(new Color(0x06, 0x03, 0x01, 225));
            g2.fillRoundRect(mX, mY, mW, mH, 12, 12);
            float mAlpha = matchTimeLeft <= 15
                    ? 130 + 125*(float)Math.abs(Math.sin(glowTick*2.5f)) : 185;
            ...
            if (turnCountdownTimer != null && turnCountdownTimer.isRunning()) {
                ...
                int tW = tfm.stringWidth(tTxt) + padH * 2;
                int tH = tfm.getHeight() + padV * 2;
                ...
                float tAlpha = timeLeft <= 3
                        ? 120 + 135*(float)Math.abs(Math.sin(glowTick*3)) : 155;
                ...
                int tTX = tX + (tW - tfm.stringWidth(tTxt)) / 2;
                int tTY = tY + padV + tfm.getAscent();
