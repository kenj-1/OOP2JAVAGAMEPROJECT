...
public class PVPBattleFrame extends JFrame {
    ...
    // Full-art images for P2 selection cards (matches CharacterSelectionFrame)
    ...
    // Small portrait images for the battle HUD boxes
    private static final String[] PORTRAIT_IMGS = {
    ...
    // ══════════════════════════════════════════════════════════
    //  Phase 1 — P2 Selection
    // ══════════════════════════════════════════════════════════
    private JPanel buildP2SelectPanel() {
        JPanel wrapper = new JPanel(new BorderLayout());
        wrapper.setBackground(Color.BLACK);
        selectPane = new JLayeredPane();
        ...
        // P2 selection countdown badge
        JPanel p2Badge = new JPanel() {
            @Override protected void paintComponent(Graphics g) {
                super.paintComponent(g);
                if (p2SelectCountdownTimer == null || !p2SelectCountdownTimer.isRunning()) return;
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                g2.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
                int W = getWidth(), H = getHeight();
                Color base = p2SelectTimeLeft > 9  ? new Color(0xC8,0xA0,0x28)
                        : p2SelectTimeLeft > 4  ? new Color(0xCC,0x66,0x11)
                        :                         new Color(0xCC,0x22,0x22);
                float pulse = p2SelectTimeLeft <= 4
                        ? 1f + 0.07f * (float)Math.abs(Math.sin(p2SelectPulse)) : 1f;
                int bw=(int)(W*0.86),bh=H-4,bx=(W-bw)/2,by=2;
                ...
                int lblSz=Math.max(9,(int)(H*0.22));
                g2.setFont(new Font("Serif",Font.ITALIC,lblSz));
                FontMetrics lf=g2.getFontMetrics(); String lbl="P2 AUTO-PICK IN";
                g2.setColor(new Color(base.getRed(),base.getGreen(),base.getBlue(),155));
                g2.drawString(lbl,(W-lf.stringWidth(lbl))/2,by+lf.getAscent()+3);
                int numSz=(int)(Math.max(20,H*0.52)*pulse);
                g2.setFont(new Font("Serif",Font.BOLD,numSz));
                FontMetrics nf=g2.getFontMetrics();
                String full="\u25C6  "+p2SelectTimeLeft+"  \u25C6";
                int nx=(W-nf.stringWidth(full))/2,ny=by+lf.getHeight()+nf.getAscent()+2;
                g2.setColor(new Color(0,0,0,160)); g2.drawString(full,nx+2,ny+2);
                if (p2SelectTimeLeft<=4) {
                    for(int r=3;r>=1;r--){g2.setColor(new Color(base.getRed(),base.getGreen(),base.getBlue(),18*(4-r)));g2.drawString(full,nx-r,ny);g2.drawString(full,nx+r,ny);}
                }
                g2.setColor(base); g2.drawString(full,nx,ny); g2.dispose();
            }
        };
        p2Badge.setOpaque(false);
        selectPane.add(p2Badge, JLayeredPane.POPUP_LAYER);
        ...
        selectPane.addComponentListener(new ComponentAdapter() {
            @Override public void componentResized(ComponentEvent e) {
                ...
                if(W==0||H==0) return;
                double sc=Math.min(Math.min(W/1024.0,H/768.0),1.5);
                int bw=(int)(200*sc),bh=(int)(52*sc);
                p2Badge.setBounds((W-bw)/2,(int)(5*sc),bw,bh);
            }
        });
    ...
    private void layoutSelectPane() {
        ...
        for (int i=0;i<8;i++){int fx=gridX+(i%4)*(cellW+gapX),fy=gridY+(i/4)*(cellH+gapY);cards[i].setBounds(fx,fy,cellW,cellH);baseY[i]=fy;}
    ...
    private void startP2SelectTimer() {
        stopP2SelectTimer();
        p2SelectTimeLeft = 15; p2SelectPulse = 0f;
        p2SelectPulseTimer = new Timer(16, e -> { p2SelectPulse += 0.10f; if(selectPane!=null)selectPane.repaint(); });
        p2SelectPulseTimer.start();
        p2SelectCountdownTimer = new Timer(1000, e -> {
            p2SelectTimeLeft--;
            if (selectPane != null) selectPane.repaint();
            if (p2SelectTimeLeft <= 0) { stopP2SelectTimer(); SwingUtilities.invokeLater(this::autoPickP2); }
        });
    ...
    private void autoPickP2() {
        for (Character c : ROSTER) { if (!c.getName().equals(player1.getName())) { onP2Selected(c); return; } }
    }
    ...
    private JPanel buildCoinTossPanel() {
        ...
        coinFaceLabel.setBorder(BorderFactory.createCompoundBorder(new LineBorder(BORDER_CLR,3),BorderFactory.createEmptyBorder(16,44,16,44)));
    ...
    private void animateCoin() {
        ...
        anim.addActionListener(ev->{
            if(idx[0]<sl.length){coinFaceLabel.setText(sl[idx[0]]);coinFaceLabel.setForeground(idx[0]%2==0?GOLD:P2_CLR);idx[0]++;}
            else{anim.stop();coinFaceLabel.setText(result.equals("HEADS")?"H":"T");coinFaceLabel.setForeground(GOLD);
                ...
                new Timer(2000,done->{((Timer)done.getSource()).stop();launchBattle();}){{setRepeats(false);start();}};}
        });
        anim.start();
    }
    ...
    private JPanel buildBattlePanel() {
        ...
        skillsLayer=new JPanel(null); skillsLayer.setOpaque(false);
        buildSkillsLayer(); registerHotkeys(lp); lp.add(skillsLayer,JLayeredPane.MODAL_LAYER);
        ...
        lp.addComponentListener(new ComponentAdapter(){
            @Override public void componentResized(ComponentEvent e){
                int W=lp.getWidth(),H=lp.getHeight(); if(W==0||H==0)return;
    ...
    private void registerHotkeys(JComponent battleRoot) {
        ...
        for(int[]kb:p1Keys){int kc=kb[0];int si=kb[1];String id="pvp_p1_skill_"+si;
            im.put(KeyStroke.getKeyStroke(kc,0,false),id);
            am.put(id,new AbstractAction(){@Override public void actionPerformed(java.awt.event.ActionEvent e){if(si<p1SkillBtns.length&&p1SkillBtns[si]!=null&&p1SkillBtns[si].isEnabled())onP1Skill(si);}});}
        int[][] p2Keys={{KeyEvent.VK_NUMPAD1,0},{KeyEvent.VK_NUMPAD2,1},{KeyEvent.VK_NUMPAD3,2}};
        for(int[]kb:p2Keys){int kc=kb[0];int si=kb[1];String id="pvp_p2_skill_"+si;
            im.put(KeyStroke.getKeyStroke(kc,0,false),id);
            am.put(id,new AbstractAction(){@Override public void actionPerformed(java.awt.event.ActionEvent e){if(si<p2SkillBtns.length&&p2SkillBtns[si]!=null&&p2SkillBtns[si].isEnabled())onP2Skill(si);}});}
    }

    private void buildSkillsLayer() {
        List<Skill> s1=player1.getSkills(); List<Skill> s2=player2!=null?player2.getSkills():List.of();
        ...
        for(int i=0;i<3;i++){
            final int si=i;
            String n1=(i<s1.size())?s1.get(i).getName():"—"; String n2=(i<s2.size())?s2.get(i).getName():"—";
            p1SkillBtns[i]=makePillButton(n1,new Color(0x70,0x14,0x14),new Color(0xFF,0x99,0x99));
            p1SkillBtns[i].setEnabled(false); p1SkillBtns[i].addActionListener(e->onP1Skill(si));
            p1CdLabels[i]=makeCdLabel("READY",GREEN_RDY);
            p1Panel.add(makeSkillSlot(p1SkillBtns[i],p1CdLabels[i])); if(i<2)p1Panel.add(Box.createVerticalStrut(4));
            p2SkillBtns[i]=makePillButton(n2,new Color(0x14,0x40,0x28),new Color(0x99,0xFF,0xBB));
            p2SkillBtns[i].setEnabled(false); p2SkillBtns[i].addActionListener(e->onP2Skill(si));
            p2CdLabels[i]=makeCdLabel("READY",GREEN_RDY);
            p2Panel.add(makeSkillSlot(p2SkillBtns[i],p2CdLabels[i])); if(i<2)p2Panel.add(Box.createVerticalStrut(4));
        }
        skillsLayer.add(p1Panel); skillsLayer.add(p2Panel);
        skillsLayer.putClientProperty("p1",p1Panel); skillsLayer.putClientProperty("p2",p2Panel);
        battleLog=new JTextArea();
        battleLog.setFont(new Font("Monospaced",Font.PLAIN,13)); // increased from 10
        ...
        JPanel logHolder=new JPanel(new BorderLayout()){
            @Override protected void paintComponent(Graphics g){
                Graphics2D g2=(Graphics2D)g.create(); g2.setColor(new Color(0,0,0,145)); g2.fillRoundRect(0,0,getWidth(),getHeight(),10,10);
    ...
    private void onP1Skill(int si){
        ...
        turnManager.advanceTurn(); refreshAllCdLabels(); updateTurnState(); processingTurn=false;
    }
    ...
    private void endRound(boolean p1Won){
        ...
        if(p1Wins>=ROUNDS_TO_WIN||p2Wins>=ROUNDS_TO_WIN){
            new Timer(1600,e->{((Timer)e.getSource()).stop();showMatchResult(p1Wins>=ROUNDS_TO_WIN);}){{setRepeats(false);start();}};
        }else{currentRound++;new Timer(2000,e->{((Timer)e.getSource()).stop();startNextRound();}){{setRepeats(false);start();}};}
    }
    ...
    private void showMatchResult(boolean p1Won){
        ...
        SwingUtilities.invokeLater(()->{
            ...
            else{boolean wWants=(winVote==0);rematch=Math.random()<WINNER_VOTE_WEIGHT?wWants:!wWants;
    ...
    private void onTurnTimerExpired(){
        ...
        boolean wasP1=turnManager.isPlayerTurn();
        log("⏰  Time's up!  "+(wasP1?player1.getName()+" (P1)":player2.getName()+" (P2)")+"'s turn forfeited.");
    ...
    private void onMatchTimerExpired(){
        stopTurnTimer(); setP1Enabled(false); setP2Enabled(false);
        log("⏱  TIME OVER!");
        if(player1.getCurrentHP()>player2.getCurrentHP()){log("🏆  "+player1.getName()+" (P1) wins by HP advantage!");endRound(true);}
        else if(player2.getCurrentHP()>player1.getCurrentHP()){log("🏆  "+player2.getName()+" (P2) wins by HP advantage!");endRound(false);}
    ...
    private void refreshCdRow(Character ch,JButton[]btns,JLabel[]lbls){
        ...
        for(int i=0;i<btns.length;i++){if(btns[i]==null||i>=skills.size())continue;
    ...
    private void setP1Enabled(boolean on){for(JButton b:p1SkillBtns)if(b!=null)b.setEnabled(on);if(on)refreshCdRow(player1,p1SkillBtns,p1CdLabels);}
    private void setP2Enabled(boolean on){for(JButton b:p2SkillBtns)if(b!=null)b.setEnabled(on);if(on&&player2!=null)refreshCdRow(player2,p2SkillBtns,p2CdLabels);}
    ...
    private class BattleCanvas extends JPanel {
        ...
        @Override protected void paintComponent(Graphics g){
            ...
            int W=getWidth(),H=getHeight(); double sc=Math.min(W/1024.0,H/768.0);
            Graphics2D g2=(Graphics2D)g.create();
            g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);
            ...
            drawShadow(g2,"HP: "+player1.getCurrentHP()+" / "+player1.getMaxHP(),p1hpx,p1hpy+hpH+(int)(10*sc),new Color(0xFF,0xF5,0xDC,190));
            ...
            g2.setFont(new Font("SansSerif",Font.PLAIN,Math.max(8,(int)(10*sc))));
            drawShadow(g2,"HP: "+player2.getCurrentHP()+" / "+player2.getMaxHP(),p2hpx,p2hpy+hpH+(int)(10*sc),new Color(0xFF,0xF5,0xDC,190));
            ...
            drawTimerBadge(g2,W,sc);
            drawMatchTimerBadge(g2,W,H,sc);
            g2.dispose();
        }

        private void drawTimerBadge(Graphics2D g2,int W,double sc){
            if(turnCountdownTimer==null||!turnCountdownTimer.isRunning())return;
            boolean p1Turn=(turnManager!=null&&turnManager.isPlayerTurn());
            Color timerColor=timeLeft>6?new Color(0x40,0xCC,0x40):timeLeft>3?ORANGE_LOW:RED_CRIT;
            String label=(p1Turn?"P1 ":"P2 ")+timeLeft;
            g2.setFont(new Font("Serif",Font.BOLD,Math.max(13,(int)(17*sc))));
            FontMetrics fm=g2.getFontMetrics();
            int padH=(int)(10*sc),padV=(int)(5*sc);
            int bw=fm.stringWidth(label)+padH*2,bh=fm.getHeight()+padV;
            int bx=(W-bw)/2,tabH=(int)(68*sc),by=(int)(6*sc)+tabH+(int)(28*sc);
            g2.setColor(new Color(0x08,0x04,0x02,215));g2.fillRoundRect(bx,by,bw,bh,bh,bh);
            float borderAlpha=timeLeft<=3?120+135*(float)Math.abs(Math.sin(glowTick*3)):150;
            g2.setColor(new Color(timerColor.getRed(),timerColor.getGreen(),timerColor.getBlue(),Math.min(255,(int)borderAlpha)));
            g2.setStroke(new BasicStroke(2f));g2.drawRoundRect(bx,by,bw,bh,bh,bh);
            int tx=bx+(bw-fm.stringWidth(label))/2,ty=by+fm.getAscent()+padV/2;
            g2.setColor(new Color(0,0,0,130));g2.drawString(label,tx+1,ty+1);
            g2.setColor(timerColor);g2.drawString(label,tx,ty);
        }

        private void drawMatchTimerBadge(Graphics2D g2,int W,int H,double sc){
            if(matchTimer==null||!matchTimer.isRunning())return;
            Color barColor=matchTimeLeft>40?new Color(0x40,0xCC,0x40):matchTimeLeft>15?ORANGE_LOW:RED_CRIT;
            int barH=(int)(8*sc),barY=H-(int)(152*sc);
            int barW=(int)(W*0.62),barX=(W-barW)/2;
            g2.setColor(new Color(0x08,0x04,0x02,200));g2.fillRoundRect(barX,barY,barW,barH,barH,barH);
            int filled=(int)(barW*((double)matchTimeLeft/MATCH_DURATION_SECONDS));
            if(filled>2){g2.setPaint(new GradientPaint(barX,barY,barColor.brighter(),barX,barY+barH,barColor.darker()));g2.fillRoundRect(barX,barY,filled,barH,barH,barH);}
            float borderAlpha=matchTimeLeft<=15?130+125*(float)Math.abs(Math.sin(glowTick*2.5f)):140;
            g2.setStroke(new BasicStroke(1.5f));g2.setColor(new Color(barColor.getRed(),barColor.getGreen(),barColor.getBlue(),Math.min(255,(int)borderAlpha)));
            g2.drawRoundRect(barX,barY,barW,barH,barH,barH);
            int numSize=Math.max(10,(int)(12*sc));g2.setFont(new Font("Serif",Font.BOLD,numSize));
            FontMetrics fm=g2.getFontMetrics();String txt=matchTimeLeft+"s";
            int tx=barX+barW+(int)(6*sc),ty=barY+fm.getAscent()-1;
            g2.setColor(new Color(0,0,0,130));g2.drawString(txt,tx+1,ty+1);g2.setColor(barColor);g2.drawString(txt,tx,ty);
        }
    ...
    private JButton makeDarkButton(String text){
        ...
        btn.setContentAreaFilled(false);btn.setBorderPainted(false);btn.setFocusPainted(false);
        btn.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));btn.setPreferredSize(new Dimension(120,40));return btn;
    }
    ...
    private class CharCard extends JPanel{private final Image img;private final String name;private final Color glow;private boolean hovered=false;private float glowAlpha=0f;private final Timer glowTimer;CharCard(String path,String name,Color glow){this.img=loadImage(path);this.name=name;this.glow=glow;setOpaque(false);glowTimer=new Timer(20,e->{if(hovered&&glowAlpha<1f){glowAlpha=Math.min(1f,glowAlpha+0.08f);repaint();}else if(!hovered&&glowAlpha>0f){glowAlpha=Math.max(0f,glowAlpha-0.06f);repaint();}});glowTimer.start();}void setHovered(boolean h){this.hovered=h;}@Override protected void paintComponent(Graphics g){super.paintComponent(g);Graphics2D g2=(Graphics2D)g.create();g2.setRenderingHint(RenderingHints.KEY_INTERPOLATION,RenderingHints.VALUE_INTERPOLATION_BILINEAR);g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING,RenderingHints.VALUE_ANTIALIAS_ON);int W=getWidth(),H=getHeight();if(glowAlpha>0f){for(int r=4;r>=1;r--){int sp=r*6;float a=(glowAlpha*0.25f)/r;g2.setColor(new Color(glow.getRed(),glow.getGreen(),glow.getBlue(),Math.min(255,(int)(a*255))));g2.setStroke(new BasicStroke(sp));g2.drawRoundRect(sp/2,sp/2,W-sp,H-sp,16,16);}}float sc=hovered?1.08f:1.0f;int dw=(int)(W*sc),dh=(int)(H*sc),dx=(W-dw)/2,dy=(H-dh)/2;if(img!=null)g2.drawImage(img,dx,dy,dw,dh,null);else{g2.setColor(new Color(60,50,30));g2.fillRoundRect(dx,dy,dw,dh,12,12);}if(glowAlpha>0.05f){int fs=Math.max(10,(int)(H*0.13));g2.setFont(new Font("SansSerif",Font.BOLD,fs));FontMetrics fm=g2.getFontMetrics();int tx=(W-fm.stringWidth(name))/2,ty=H-(int)(H*0.06);int pp=6,pw=fm.stringWidth(name)+pp*2,ph=fm.getHeight()+2;g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,glowAlpha*0.85f));g2.setColor(glow.darker());g2.fillRoundRect((W-pw)/2,ty-fm.getAscent()-2,pw,ph,ph,ph);g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,glowAlpha));g2.setColor(new Color(0,0,0,160));g2.drawString(name,tx+1,ty+1);g2.setColor(Color.WHITE);g2.drawString(name,tx,ty);}g2.dispose();}}
